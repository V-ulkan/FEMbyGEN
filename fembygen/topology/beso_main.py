class BesoMain:
    ...
    def main(self,analysis):
        ...
        os.putenv('OMP_NUM_THREADS', str(self.doc.Topology.cpu_cores))
        ...
        [nodes, Elements, domains, opt_domains, en_all, plane_strain, plane_stress, axisymmetry] = beso_lib.import_inp(
            file_name, self.domains_from_config, self.domain_optimized, self.shells_as_composite)
        domain_shells = {}
        domain_volumes = {}
        ...
        if isinstance(self.continue_from, int):
            ...
            print("girdi5")
            elm_states = beso_lib.import_frd_state(self.continue_from, elm_states, self.number_of_states, file_name)
        elif self.continue_from[-4:] == ".inp":
            print("girdi6")
            elm_states = beso_lib.import_inp_state(self.continue_from, elm_states, self.number_of_states, file_name)
        elif self.continue_from[-4:] == ".csv":
            print("girdi7")
            elm_states = beso_lib.import_csv_state(self.continue_from, elm_states, file_name)
        else:
            for dn in self.domains_from_config:
                for en in domains[dn]:
        ...
        if iterations_limit == "auto":  # automatic setting
            print("girdi9")
            m = mass[0] / mass_full
            if self.ratio_type == "absolute" and (self.mass_removal_ratio - mass_addition_ratio > 0):
                print("girdi10")
        ...
        while True:
            if FreeCADGui.activeself.document() == None:
                print("girdi35")
